  ...
  return (
    <div>
      ...
      {sections.map((section, index) => {
        ...
        const institute = section.institute;
        ...
        const isAnyFieldFilled = course || institute || startDate || endDate;
                ...
                {isAnyFieldFilled ? (
                  <>
                    <p className="flex justify-between">
                      <span className='font-[600]'>{`${course || 'Course'} from ${institute || 'Institute'}`}</span>
                    </p>
              ...
              <div className="flex flex-col gap-5 mt-3">
                <div className="flex gap-7 w-full flex-col lg:flex-row lg:flex-nowrap">
                  <div className="flex flex-col gap-2 w-full max-w-[360px]">
                    <label className="text-[#999999] text-[14px]">Course</label>
                    <input
                      type="text"
                      className="rounded border-[1px] p-[10px] px-[16px] font-[500] border-gray"
                      value={section.course}
                      onChange={(e) => handleInputChange(index, e, 'course')}
                      placeholder=""
                    />
                  </div>
                  <div className="flex flex-col gap-2 w-full max-w-[360px]">
                    <label className="text-[#999999] text-[14px]">Institute</label>
                    <input
                      type="text"
                      className="rounded border-[1px] p-[10px] px-[16px] font-[500] border-gray"
                      value={section.institute}
                      onChange={(e) => handleInputChange(index, e, 'institute')}
                      placeholder=""
                    />
                  </div>
                </div>
      ...
      <button
        ...
        onClick={addSection}
      >
        <AddSectionBtn text="Certifications" />
      </button>
    </div>
  );
};
